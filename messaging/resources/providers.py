...
class Provider(Resource):
    ...
    def put(self, id):
        return compose(
            partial(helpers.make_update(providers.Provider, update_fields), id),
            request.get_json,
        )()
...
class ProviderList(Resource):
    ...
    def post(self):
        return (
            compose(
                helpers.make_create(providers.Provider, create_fields, id_field),
                request.get_json,
            )(),
            201,
        )
...
single_method_fields = merge(pick(["name", "base_url"], resource_fields), method_fields)
...
class ProviderMethodPut(Resource):
    @marshal_with(single_method_fields)
    def post(self, id):
        return compose(partial(providers.put_method, id), method_parser.parse_args)()
...
class ProviderConfigPut(Resource):
    ...
    def post(self, id):
        return compose(partial(providers.put_config, id), config_parser.parse_args)()
